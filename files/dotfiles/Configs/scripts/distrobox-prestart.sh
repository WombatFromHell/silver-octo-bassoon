...
is_container_running() {
  ...
  # Also check that it's NOT in "Exited" state
  if distrobox list | tail -n +2 | grep -q "| ${container_name}[[:space:]]*|.*\<running\>\|.*\<Up\>"; then
    return 0 # Container is running
  fi

  # If container is in "Exited" state, it's not running
  if distrobox list | tail -n +2 | grep -q "| ${container_name}[[:space:]]*|.*\<Exited\>"; then
    return 1 # Container is exited, needs to be started
  fi

  return 1 # Container is not running (other state)
}
